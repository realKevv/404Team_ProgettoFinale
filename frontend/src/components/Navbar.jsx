...
export default function Navbar({ onMenuClick }) {
    ...
    return (
      trasferte
        ...
        .map((trip) => {
          ...
          if (trip.stato === "approvata")
            messaggio = `L'admin ha approvato la tua trasferta a ${trip.destinazione}! 🎉`;
          if (trip.stato === "rifiutata")
            messaggio = `La richiesta per ${trip.destinazione} è stata rifiutata dall'admin.`;
  ...
  return (
    <nav
      ...
      style={{
        backgroundColor:
          "color-mix(in srgb, var(--colore-sfondo-card) 80%, transparent)",
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2" ref={notifRef}>
            ...
            <Link
              ...
              className="hover-tema flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200"
              style={{
            ...
            <div className="relative">
              ...
              <button
                ...
                className={`relative p-2 rounded-xl hover-tema transition-colors cursor-pointer ${isNotifOpen ? "bg-[var(--colore-sfondo-alt)]" : ""}`}
              >
                {mostraPallinoRosso ? (
                  <BellRing
                    size={20}
              ...
              {isNotifOpen && (
                <div
                  className="absolute right-0 mt-2 w-80 sm:w-96 max-w-[calc(100vw-2rem)] rounded-2xl shadow-xl z-50 overflow-hidden"
                  style={{
                ...
                >
                  ...
                  <div className="p-4 border-b flex justify-between items-center" style={{ borderColor: "var(--colore-bordo)" }}>
                    <h3 className="font-semibold text-sm" style={{ color: "var(--colore-testo-principale)" }}>
                      Le tue Notifiche
                    </h3>
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y" style={{ borderColor: "var(--colore-bordo)" }}>
                    {/* SE NON CI SONO NOTIFICHE: mostra questa scritta */}
                    {userNotifications.length === 0 ? (
                      <div className="p-6 text-center text-sm" style={{ color: "var(--colore-testo-mutato)" }}>
                      ...
                      userNotifications.map((notif) => (
                        <div
                          ...
                          className="p-4 hover-tema transition-colors cursor-pointer text-xs sm:text-sm flex items-start justify-between gap-2"
                          title="Clicca per segnare come letta e rimuovere"
                          style={{ borderBottom: "1px solid var(--colore-bordo)" }}
                        >
                          {/* Testo del messaggio e scritta "Nuovo" o "Aggiornato" */}
                          <div className="flex-1">
                            <p className="leading-relaxed font-medium" style={{ color: "var(--colore-testo-principale)" }}>
                              {notif.text}
                            </p>
                            <span className="text-[10px] block mt-1" style={{ color: "var(--colore-testo-mutato)" }}>
                              {notif.time}
                            </span>
                          </div>
                          {/* Pallino */}
                          <span
                            ...
                            style={{
                              backgroundColor: "var(--colore-pericolo)",
                            }}
                          />
                        </div>
                      ))
